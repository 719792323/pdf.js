...
class FloatingOutline {
   ...
   */
  constructor({ container, outlineView, eventBus, viewerContainer }) {
  ...
  popOut(mouseX, mouseY) {
    ...
    // 使用保存的尺寸或默认尺寸
    const panelWidth = this.#hasSavedSize ? this.#savedWidth : 300;
    const panelHeight = this.#hasSavedSize ? this.#savedHeight : 400;
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;

    let left, top;

    // 如果提供了鼠标位置，则面板出现在鼠标位置（鼠标在面板中心）
